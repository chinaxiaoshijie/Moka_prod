...
import { formatUTCToLocal, formatUTCToShort, formatDateOnly, formatTimeOnly } from "@/lib/timezone";
...
export default function InterviewDetailPage() {
  ...
  const [emailSentAt, setEmailSentAt] = useState<Date | null>(() => {
    try {
      const saved = localStorage.getItem(`email_sent_${interviewId}`);
  ...
  // 当 emailContent 变化时，加载到编辑器
  useEffect(() => {
    if (!showEmailModal) {
      contentLoadedRef.current = false; // 模态框关闭时重置
      return;
    }
    
    if (!emailContent || emailContent.length === 0) return;
    if (contentLoadedRef.current) return; // 避免重复加载
    
    const loadContent = () => {
      console.log('[Quill] Trying to load content, length:', emailContent.length, 'editor ready:', !!quillInstance.current);
      
      if (!quillInstance.current) {
        console.log('[Quill] Waiting for editor initialization...');
        // 编辑器还没准备好，等待一下再试
        setTimeout(loadContent, 50);
        return;
      }
      
      console.log('[Quill] Loading content into editor...');
      quillInstance.current.root.innerHTML = emailContent;
      contentLoadedRef.current = true;
      console.log('[Quill] ✅ Content loaded successfully');
      console.log('[Quill] Editor innerHTML preview:', quillInstance.current.root.innerHTML.substring(0, 100));
    };
  ...
  const handleSendEmailToCandidate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const now = new Date();
        setEmailSentAt(now);
        localStorage.setItem(`email_sent_${interviewId}`, now.toISOString());
              ...
              <div className="space-y-2">
                ...
                <button
                  onClick={() => {
                    ...
                    // 格式化面试时间
                    const formattedStartTime = formatDateTime(interview.startTime);
                    const formattedDate = formatDate(interview.startTime);
                    const formattedEndTime = formatTime(interview.endTime);
                    
                    console.log('[Email] Interview time:', interview.startTime, '→ Display:', formattedStartTime);
                    
                    // 生成邮件内容（使用 Quill 支持的格式）
                    const emailHtmlContent = `<p>尊敬的 <strong>${interview.candidate.name}</strong> 您好：</p>
<p>感谢您应聘我司 <strong>${interview.position.title}</strong> 职位。经过初步筛选，我们诚挚邀请您参加面试：</p>
<p><br></p>
<h3>面试信息</h3>
...
<p>祝您面试顺利！</p>`;
                    ...
                    console.log('[Email] Content length:', emailHtmlContent.length);
                    
                    // 设置邮件内容
                    setEmailSubject(`面试通知 - ${interview.position.title}`);
                    setEmailContent(emailHtmlContent);
                ...
                >
                  ...
                  {emailSentAt && (
                    <p className="text-xs text-[#999] mt-1">📧 上次发送：{Math.floor((Date.now() - emailSentAt.getTime()) / 3600000)} 小时前</p>
                  )}
                </button>
              </div>
            </div>
